...
// attempt.
func (m *recoveryManager) resurrect() error {
	if err := m.extendScanAccounts(); err != nil {
		return err
	}

	//generate resurrect address.
	for _, state := range m.state.AccountsStatus {
		curHorizon, delta := state.InternalBranch.extendHorizon()
		for index := curHorizon; index < curHorizon+delta; index++ {
			if err := m.extendAddress(state.Account, index, true); err != nil {
...
func (m *recoveryManager) saveAccount(account *account.Account) error {
	account, err := m.accountMgr.FindByID(account.ID)
	if err != nil {
		return err
	}
	...
	if err := m.accountMgr.SaveAccount(account); err != nil {
		return err
	}
